...
export default {
  ...
  setup() {
    ...
    const addEmployee = () => {
      store
        .dispatch("app-employees/add", { model })
        .then((response) => {
          userData.value = response.data;
          toast({
            component: ToastificationContent,
            props: {
              title: `The employee ${model.value.name} was created correctly`,
        ...
        .catch((error) => {
          ...
          if (error.response.status === 409) {
            toast({
              component: ToastificationContent,
              props: {
                title: `the identification number used already exists : ${model.value.identification_number}`,
                icon: "AlertTriangleIcon",
                variant: "danger",
              },
            });
